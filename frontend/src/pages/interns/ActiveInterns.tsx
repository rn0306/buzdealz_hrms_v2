...
interface Intern {
  ...
  // INTERN-ONLY FIELDS (now handled in another module; kept only in type)
  work_type?: string;
...
const ActiveInterns: React.FC = () => {
  ...
  const [extensionInternId, setExtensionInternId] = useState<string | null>(
    null
  );
  const [extensionForm, setExtensionForm] = useState<{
    ...
    newEndDate: string;
    old_end_date?: string;
  }>({ reason: "", newEndDate: "", old_end_date: "" });

  const [showTerminationModal, setShowTerminationModal] = useState(false);
  const [terminationInternId, setTerminationInternId] = useState<string | null>(
    null
  );
  ...
  const [updatingExtensionId, setUpdatingExtensionId] =
    ...
    async function load() {
      try {
        ...
        const mapped: Intern[] = data.map((d: any) => {
          const user = d.user || {};
          const roleCode =
            rolesData.find((r) => r.id === user.role_id)?.code || "";
            ...
            mapped.map(async (intern) => {
              try {
                const extRes = await api.get(
                  `/api/extensions/user/${intern.id}`
                );
                const termRes = await api.get(
                  `/api/terminations/user/${intern.id}`
                );
  ...
  const managerOptions = useMemo(
    () => interns.filter((emp) => emp.Role.code === "MANAGER"),
    [interns]
  );
  ...
  const shouldShowManagerDropdown = (role: string | undefined) =>
    role === "INTERN";
  ...
  async function handleEditIntern(edited: Partial<Intern>, original: Intern) {
    try {
      ...
      // user fields
      if (edited.fname || edited.lname)
        payload.full_name = `${edited.fname || original.fname} ${
          edited.lname || original.lname
        }`.trim();
      ...
      // Manager assignment
      if ((edited as any).manager_id !== undefined)
        payload.manager_id = (edited as any).manager_id;
      ...
      const updatedIntern: Intern = {
        ...
        personalDetail: {
          ...
          ...updatedDetail,
          verification_status:
            updatedDetail.verification_status ||
            original.personalDetail.verification_status,
        },
  ...
  async function handleRequestExtension() {
    ...
    try {
      // old_end_date is optional now (internship duration handled elsewhere)
      const oldEnd = extensionForm.old_end_date || "";
  ...
  async function handleTerminateIntern() {
    ...
    try {
      ...
      const termRes = await api.get(
        `/api/terminations/user/${terminationInternId}`
      );
  ...
  const canApprove =
    loggedInUser?.role === "ADMIN" || loggedInUser?.role === "RECRUITER";
  ...
  async function updateExtensionStatus(
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.error || "Failed to update extension status"
      );
    } finally {
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-6">
            ...
            {filteredInterns.map((intern) => {
              ...
              return (
                <TR key={intern.id}>
                  ...
                  <TD>
                    {intern.join_date
                      ? new Date(intern.join_date).toLocaleDateString("en-IN")
                      : "-"}
                  </TD>
                  {/* <TD>{managerName}</TD> */}
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <span
                          className={`px-2 py-1 rounded text-xs font-semibold ${
                            extStatus.status?.toUpperCase() === "PENDING"
                              ? "bg-yellow-100 text-yellow-800"
                              : extStatus.status?.toUpperCase() === "APPROVED"
                          ...
                          extStatus.status?.toUpperCase() === "PENDING" && (
                            <div className="flex gap-2 mt-1">
                              <Button
                                ...
                                disabled={
                                  updatingExtensionId === extStatus.id
                                }
                              >
                                Approve
                              </Button>
        ...
        >
          <div className="flex gap-3">
            <Input
              ...
              value={addForm.fname || ""}
              onChange={(e) =>
                setAddForm((f) => ({ ...f, fname: e.target.value }))
              }
            />
            <Input
              label="Last Name"
              ...
              value={addForm.lname || ""}
              onChange={(e) =>
                setAddForm((f) => ({ ...f, lname: e.target.value }))
              }
            />
          </div>
          ...
          <Select
            ...
            label="Role"
            required
            value={addForm.Role?.code || ""}
            onChange={(e) =>
              setAddForm((f) => ({ ...f, Role: { code: e.target.value } }))
            }
          >
          ...
          <Input
            ...
            onChange={(e) =>
              setAddForm((f) => ({ ...f, join_date: e.target.value }))
            }
          />
          ...
          {shouldShowManagerDropdown(addForm.Role?.code) && (
            <Select
              ...
              onChange={(e) =>
                setAddForm((f) => ({ ...f, manager_id: e.target.value }))
              }
            >
        ...
        {showEditModal && (
          ...
          >
            ...
            <Select
              ...
              onChange={(e) =>
                setEditForm((f) => ({ ...f, Role: { code: e.target.value } }))
              }
            >
              <option value="">Select Role</option>
              {roles.map((r: RoleType) => (
            ...
            <Input
              ...
              onChange={(e) =>
                setEditForm((f) => ({ ...f, join_date: e.target.value }))
              }
            />
            ...
            {shouldShowManagerDropdown(editForm.Role?.code) && (
              <Select
                ...
                onChange={(e) =>
                  setEditForm((f) => ({ ...f, manager_id: e.target.value }))
                }
              >
                <option value="">Select Manager</option>
                {managerOptions.map((m) => (
      ...
      <Dialog
        ...
        title={
          selectedIntern
            ? `${selectedIntern.fname} ${selectedIntern.lname} Details`
            : ""
        }
      >
        {selectedIntern && (
          <div className="flex flex-col gap-2 text-sm">
            <div>
              <strong>Full Name:</strong> {selectedIntern.fname}{" "}
              {selectedIntern.lname}
            </div>
            <div>
            ...
            <div>
              ...
              {selectedIntern.personalDetail.highest_education}
            </div>
          </div>
        )}
      </Dialog>
      ...
      <Dialog
        ...
        title="Request Internship Extension"
      >
        <form
          className="flex flex-col gap-4"
          onSubmit={(e) => {
        ...
        >
          ...
          <textarea
            ...
            onChange={(e) =>
              setExtensionForm((f) => ({ ...f, reason: e.target.value }))
            }
          ...
          <Input
            ...
            onChange={(e) =>
              setExtensionForm((f) => ({ ...f, newEndDate: e.target.value }))
            }
            required
          />
      ...
      >
        ...
        >
          <div className="bg-red-50 border border-red-200 rounded p-3 text-sm text-red-700">
            <strong>Warning:</strong> This action cannot be undone. The intern
            will be marked as terminated.
          </div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setTerminationForm((f) => ({ ...f, reason: e.target.value }))
            }
